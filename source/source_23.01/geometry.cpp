...
int CGeometry::readRadiuses(char fpattern[500], int iSector, double iRad)
{
		...
		if(iRad > 0.)
		{
			for(int i=0; i< CGeometry::nRadiuses[iSector]; i++)
			{
				CGeometry::outer_radius[iSector][i] += iRad;
			}
			CGeometry::inRadius[iSector] = iRad;
		}
		return 1;
}
